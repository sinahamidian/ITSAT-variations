...
void AddNegCycleAutomaton_General(const vector<PlanEvent> &negCycle, int T, SatLayer *solver, bool **mutextable, bool *compress)
{
	...
	for (int j=0; j<totalstates; j++)
		thislvars[j]= 1 + solver->SATNewVar(0);
	//cout << endl << alltstart[totaltrans-1] << " " << alltend[totaltrans-1] << " " << totalvars;
	//exit(0);
	for (int t=0; t<T; t++)
	{
		for (int j=0; j<totalstates; j++)
			 nextlvars[j]= 1 + solver->SATNewVar(0);
		for (int j=0; j<totaltrans; j++)
			 curlvars[j]= 1 + solver->SATNewVar(0);
		for (int j=0; j<totalstates; j++)
		{
			int k;
	...
	for (int t=0; t<1; t++)
	{
		...
		int thiss=1 + solver->SATNewVar(0);
		int nexts;
		int thise=0;
		...
		if (Pos_SimilarA[0]>0)
		{
			oldtrans[noldtrans]=1 + solver->SATNewVar(0);
		...
		for (int i=0; i<SimilarA.size(); i++)
		{
			...
			while (Pos_SimilarA[i]>thise)
			{
				...
				{
					newtrans[nnewtrans]=1 + solver->SATNewVar(0);
...
//			{
			...
			{
				int endvar= 1 + solver->SATNewVar(0);


				newstates[nnewstates]= 1 + solver->SATNewVar(0);
