...
bool Alg2Layer(AlgCommonParams p)
{
	...
	PrecoSat::Solver *precosatSolver = new PrecoSat::Solver();


	vector<int> inits,goals,unitv,elimv,clss, lclss[50];
	vector<float> scores;
	int *clssc,*lclssc,*lastlclssc;
	clssc = new int;
	lclssc = new int;
	lastlclssc = new int;
	*clssc = 0;
	...
	*numOfAux = 0;
	...
	cout << endl << "Process started:" << endl << endl ;
...
T=1;
	...
	solver = new SatLayerPrecosat(
				...
				availableTime, p.gen_cnf, p.gen_readable_cnf, p.justgencnf, p.conflictsMethod, precosatSolver);
	for(T=p.TStart; T<=p.TEnd && p.AvailableTime - CPUTime() > 0;)
	{
		...
		precosatSolver->set ("print", 0);
		precosatSolver->fxopts ();
		precosatSolver->cdb=&clss;
		for(int ccc=0; ccc<50; ccc++)
		{
			precosatSolver->lcdb[ccc]=&(lclss[ccc]);
		}
		
		precosatSolver->cdbcounter=clssc;
		precosatSolver->lcdbcounter=lclssc;
		precosatSolver->lastlcdbcounter=lastlclssc;
		precosatSolver->unitv = &unitv;
		precosatSolver->inits = &inits;
		precosatSolver->goals = &goals;
		precosatSolver->elimv = &elimv;
		precosatSolver->numOfAux = numOfAux;
		precosatSolver->tempscores = &scores;

		solver->setT(T);
		checkedT.insert(T); // do not go back to this particular T, after finding a valid timed plan
		cout << "Trying with T==" << T << "... " << endl;
		availableTime = p.AvailableTime - CPUTime();
		if(availableTime > p.StageTime)
			availableTime = p.StageTime;
		precosatSolver->available_time = availableTime;
		/*if(p.satsolver == "minisat")
		{
...
//	exit(0);
				...
				if(restart_per_T == 0)
					if(res->outcome == SAT)
					{
						...
						cout << "Trying to find a satisfying timed plan:" << endl ;
					}
					else
						break;
			}
