...
SatLayer::SatLayer(
	...
	bool gen_cnf=false, bool gen_readable_cnf=false, bool dont_solve=false, int conflictsMethod=1, PrecoSat::Solver *precosatsolver=0)
	:
		...
		CONFLICTS_METHOD(conflictsMethod), precosatSolver(precosatsolver)
{
	startTime = CPUTime();
...
//	else //if(CONFLICTS_METHOD == 3)
		Old_AddMutexStart_End(mutextable,compress);cout<<"*EC5\n";
/*	for(int t=0; t<T; t++)
...
void SatLayer::AddActionVariableRelations(bool *compress)
{
	...
	for(int i=0; i<nAction; i++)
	{
		...
		{
			ResetClause();
			precosatSolver->AddInitNew(ActionTime2Var(i, 0, 2), 0);
			AddClause(ccEvent);
		}
		// ao(T-1) => ae(T-1) 
		/*if (!compress[i])
		...
		{
			//ResetClause();
			precosatSolver->AddGoalNew(ActionTime2Var(i, T, 2), 0);
			//AddClause(ccEvent);
		}
...
void SatLayer::AddInitialState()
{
	...
	{
		//ResetClause();
		precosatSolver->AddInit(PropTime2Var(pProblem.initialState[i], 0));
		//AddClause(ccInit);
...
/*	for(int i=0; i<pProblem.pAllProposition.size(); i++)
	{
		...
		{
			//ResetClause();
			precosatSolver->AddInit(PropTime2Var(i, 0), false);
			//AddClause(ccInit);
			if(GEN_CNF)
...
void SatLayer::AddGoals()
{
	...
	{
		//ResetClause();
		precosatSolver->AddGoal(PropTime2Var(pProblem.goals[i], T));
		//AddClause(ccGoal);
		if(GEN_CNF)
...
//	int *delexist = new int[nAction*2];
	...
	for(long int i=0; i<nProp; i++)
	{
		...
		////////////////////////////////////// NEW: REDUCE VARIABLES (Start)///////////////////////////////////////////
				...
				if ((j==0))
				{
					predurvar[j]=SATNewVar(0)+1;
					if (durneeded)
						delbeforevar[j]=SATNewVar(0)+1;
					//delaftervar[j]=SATNewVar(0)+1;
				}
		...
		////////////////////////////////////// NEW: REDUCE VARIABLES (End)///////////////////////////////////////////
			...
			if (durneeded)
			{
				delbeforet1var=SATNewVar(0)+1;
				delaftertvar=SATNewVar(0)+1;
			}
			///////////////////////////////////////////////NEW: Action Proposition Mutex (Start)///////////////////////////////////////////////
/*			for (int j=0; j<nAction; j++)
			{
	...
	for(int i=0; i<nProp; i++)
	{
		...
		for(int t=0; t<T; t++)
		{
			...
			predel[i][t]=SATNewVar(0)+1;

			duradd[i][t]=SATNewVar(0)+1;
			delvar[i][t]=SATNewVar(0)+1;
			delpre[i][t]=SATNewVar(0)+1;
		}
	}
